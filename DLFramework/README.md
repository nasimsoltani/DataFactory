...
The `DLFramework` saves the trained model structure (in `model_file.json`) and weights (in `model.hdf5`), saves logs (in `log.out` and `log.err`) and a `preds.pkl` file in the experiment folder.
